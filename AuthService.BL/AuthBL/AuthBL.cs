...
namespace AuthService.BL.AuthBL
{
    public class AuthBL : IAuthBL
    {
        ...
        public AuthBL(IConfiguration configuration, IServiceScopeFactory scopeFactory)
        {
        ...
        public async Task<ServiceResponse> Login(LoginRequest request)
        {
            ...
            else
            {
                using(var scope = _scopeFactory.CreateScope())
                {
                    ...
                    var user = ((await userBL.GetPagingAsync(typeof(User), new PagingRequest() {
                        PageIndex = 1, PageSize = 1, Filter = $"[[\"UserName\", \"=\", \"{request.UserName}\"], \"and\", [\"Password\", \"=\", \"{HashPassword(request.Password)}\"]]"
                    })).PageData as List<User>)?.FirstOrDefault();
                    ...
                    {
                        user = await userBL.GetByID<User>(typeof(User),user.UserID.ToString());
